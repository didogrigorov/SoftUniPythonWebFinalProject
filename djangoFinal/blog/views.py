...
def display_blog_post(request, post):
    ...
    prof_image = profile_image[1] if len(profile_image) >= 2 else "/images/none.png"
...
def create_comment(request, post):
    if request.method == 'POST':
        ...
        if form_comment.is_valid():
            ...
            author = models.CustomUser.objects.get(id=request.user.id)
...
def blog_index(request):
    context = {}
    posts = models.Post.objects.filter(status="published").order_by('-id')
    form = EmailForm()
...
class IndexView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for t in context["testimonials"]:
            test = models.CustomUser.objects.get(id=t.user.id)
